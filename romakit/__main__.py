"""CLI: romakit [input] [-o out] [--lang jp|cn|kr] [--no-foreign]
...
def main():
  ...
  parser.add_argument('--no-foreign', action='store_false', dest='foreign',
                      help='Japanese: spell katakana loanwords out instead of their English lemma')
  args = parser.parse_args()

  sys.stdin.reconfigure(encoding='utf-8')
  sys.stdout.reconfigure(encoding='utf-8')
  text = Path(args.input).read_text(encoding='utf-8') if args.input else sys.stdin.read()
  out = ''.join(romanize(line, args.lang, foreign=args.foreign) + '\n'
